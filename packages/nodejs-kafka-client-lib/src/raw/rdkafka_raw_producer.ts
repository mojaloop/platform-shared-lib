...
import { Envelope } from "../protobuff/messages";
...
/*    // synthetic sugar for type events
    ...
    private _toRDKafkaProduceParams(msg: IRawMessage): { topic: string, partition: NumberNullUndefined, message: Buffer | Uint8Array, key: Buffer, timestamp: NumberNullUndefined, headers: IRawMessageHeader[] } {
        ...
        let message: Buffer | Uint8Array = Buffer.alloc(0);
        if (msg.value instanceof Buffer) {
            message = msg.value;
        } else if (msg.value) {
            const protoBuffMessage = msg as IRawMessage & { value: { payload: any } };
            if(protoBuffMessage.value.payload instanceof Uint8Array) {
                const protoMessage = protoBuffMessage.value as any;
                const eventSerialized = protoBuffMessage.value.payload;
